			...
			<div class="item" v-for="item in commentData" :id="'item'+item.serialNO">
				<!-- <div class="avatar-content"> -->
					...
					<div class="comment-content">
						...
						<div class="edit-content" v-if="isClickEdit==item.serialNO">
							<mavon-editor
				      	ref="md1"
				       	v-model="value1"
				       	:subfield="subfield"
...
<script>
	...
	export default{
		...
		methods:{
			init(){
				var $ax=new this.$http('/sqp/communityCommentList.html','post',{
					postCode:this.$route.params.noteid,
					ajax:true,
					pagesize: this.pagesize,
					page: this.page
				},result=>{
					this.commentData=result.data.Rows;//评论区数据
        ...
        // 第一步.将图片上传到服务器.
          ...
          // $vm.$img2Url 详情见本页末尾
          this.$refs.md1.$img2Url(pos,res.data.params.filePath);
       	})
      },
      $imgAdd2(pos, $file){
        // 第一步.将图片上传到服务器.
          ...
          // 第二步.将返回的url替换到文本原位置![...](0) -> ![...](url)
          // $vm.$img2Url 详情见本页末尾
          this.$refs.md2.$img2Url(pos,res.data.params.filePath);
       	})
      },
      response(userName){//回复
            	...
            	setTimeout(()=>{
            		...
            		for(var i=0;i<this.commentData.length;i++){
            			if(this.commentData[i].serialNO==serialNO){
            				this.value1=this.commentData[i].commentContentText;
            				break;
            			}
            		}
            	},0)
      ...
      submit1(serialNO){//编辑的提交
      	// var commentContentText=;
      	var postCode=this.$route.params.noteid;
      	this.commonAjax(this.value1,postCode,'/sqp/updateCommunityComment.html',serialNO);
      	this.isClickEdit='';
      },
      cancel(serialNO){
      	this.value1='';
      ...
      submit2(){
      	var $loginAx=new this.$http('/loginStatus.html','post',{
            ...
            if(result.data.params.IsLogin=='1'){
            	this.isLogin=true;
            	var postCode=this.$route.params.noteid;
            	this.commonAjax(this.value2,postCode,'/sqp/addCommunityComment.html');
            	this.value2='';
            }else if(result.data.params.IsLogin=='0'){
            	this.isLogin=false;
              this.$router.push({
      ...
      commonAjax(commentContentText,postCode,url,serialNO){
      	var data={};
      	if(serialNO){
      		data={ajax:true,commentContentText,serialNO};
      	}else{
      		data={ajax:true,commentContentText,postCode};
      	}
